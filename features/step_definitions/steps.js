...
When("I run {string}", function (string) {
  ...
  When('I click {string} link', async function (menuOption) {
    // Write code here that turns the phrase above into concrete actions
     this.dashboardPage = poManager.getDashboardPage();
     console.log("I am automation")
     const contackMenu = this.page .locator("[title='Contacts']");
     //await this.dashboardPage("");
     await contackMenu.click();
     await contackMenu.click();
     console.log("I am automation1")
     //await this.dashboardPage.searchProductAddCart(productName);
     //await this.dashboardPage.navigateToCart();
  });

  When('I Open first contact', async function () {
    // Write code here that turns the phrase above into concrete actions
     console.log("I am automation")
     const firstContactRow = this.page .locator("[data-navigable='true']").nth(1);
     console.log("Here is the first row: " +firstContactRow)
     //await this.dashboardPage("");
     await firstContactRow.click();
     //await firstContactRow.click().nth(1);
  ...
  Given('That I login app with {string} and {string}', {timeout: 100 * 1000}, async function (username,password) {
  ...
  assert.equal(this.stdout, string)
      ...
      Given('a login to Ecommerce2 application with {string} and {string}', {timeout: 100 * 1000}, async function (username,password) {
        ...
        // page.route('**/*.{jpg,png,jpeg}',route=> route.abort());
         ...
         await this.page .locator("[type='password']").fill("secret_sauce");
         await signIn.click();   
        });
